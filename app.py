...
def resolve_db_path():
    """Resolve a writable SQLite file path.

    Railway users sometimes set DB_PATH to the mounted directory (/data)
    instead of a database filename. In that case append the default filename.
    DATA_DIR is also supported for simpler volume configuration.
    """
    raw = (os.getenv("DB_PATH") or "").strip()
    data_dir = (os.getenv("DATA_DIR") or "/data").strip() or "/data"

    if not raw:
        raw = os.path.join(data_dir, "omtogel_staff.db")
    raw = os.path.abspath(os.path.expanduser(raw))

    # A directory path, trailing slash, or extensionless /data-like path
    # means the user supplied a volume directory rather than a DB file.
    if raw.endswith(os.sep) or os.path.isdir(raw) or not os.path.splitext(os.path.basename(raw))[1]:
        raw = os.path.join(raw, "omtogel_staff.db")

    parent = os.path.dirname(raw) or "."
    os.makedirs(parent, exist_ok=True)

    # Fail early with a useful error instead of sqlite's vague message.
    probe = os.path.join(parent, ".omtogel_write_test")
    try:
        ...
        raise RuntimeError(
            f"Folder database tidak dapat ditulis: {parent}. "
            "Pasang Railway Volume ke /data dan set DB_PATH=/data/omtogel_staff.db "
            "atau DATA_DIR=/data."
        ) from exc
    return raw
...
print(f"[startup] SQLite database: {DB_PATH}", flush=True)
