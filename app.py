...
@roles('superadmin','supervisor','leader')
def channels_page():
 # Jenis jobdesk bersifat GLOBAL. Kantor hanya menentukan penempatan staf pada Jadwal Operasional.
 with db_conn() as c:
  if request.method=='POST':
...
def staff_page():
 with db_conn() as c:
  ...
  rows=c.execute('''SELECT s.*,o.name office_name,u.username login_id,u.is_active account_active,
    ...
    LEFT JOIN shift_schedules ss ON ss.staff_id=s.id AND ss.work_date=?
    LEFT JOIN shifts sh ON sh.id=ss.shift_id
    ORDER BY CASE WHEN s.status="Aktif" THEN 0 ELSE 1 END,o.name,s.name''',(report_date,)).fetchall()
  ...
  shift_counts=c.execute('''SELECT sh.id,sh.name,sh.start_time,sh.end_time,o.name office_name,COUNT(DISTINCT ss.staff_id) total
    ...
    WHERE ss.work_date=? AND s.status='Aktif' GROUP BY sh.id,sh.name,sh.start_time,sh.end_time,o.name ORDER BY o.name,sh.start_time''',(report_date,)).fetchall()
  totals={'all':len(rows),'active':len(active_rows),'ex':sum(1 for r in rows if r['status'] in ('Ex Karyawan','Resign')),'scheduled':sum(int(x['total']) for x in shift_counts)}
...
@roles('superadmin','supervisor','leader')
def offdays_page():
 flash('Menu Off Day sudah dihapus. Pengaturan staf harian sekarang dipusatkan di Jadwal Operasional.','success')
 return redirect(url_for('operations_page'))
