...
def init_db():
  with lock,db_conn() as c:
    ...
    ''')
    # migrations from v3
    for name,decl in [('exit_date','TEXT'),('exit_reason','TEXT')]: addcol(c,'staff',name,decl)
    for name,decl in [('must_change_password','INTEGER DEFAULT 1'),('twofa_secret','TEXT'),('twofa_enabled','INTEGER DEFAULT 0')]: addcol(c,'users',name,decl)
