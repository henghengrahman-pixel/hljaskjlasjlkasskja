...
def offices_page():
 with db_conn() as c:
  if request.method=='POST':
   name=request.form['name'].strip(); existing=c.execute('SELECT id FROM offices WHERE name=?',(name,)).fetchone()
   if existing: c.execute('UPDATE offices SET location=?,status=? WHERE id=?',(request.form.get('location',''),request.form.get('status','Aktif'),existing['id']))
   else: c.execute('INSERT INTO offices(name,location,status) VALUES(?,?,?)',(name,request.form.get('location',''),request.form.get('status','Aktif')))
   audit(c,'office.save',name); c.commit(); return redirect(url_for('offices_page'))
