...
def init_db():
  with lock,db_conn() as c:
    ...
    CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE NOT NULL,password_hash TEXT NOT NULL,role TEXT DEFAULT 'staff',staff_id INTEGER UNIQUE,office_id INTEGER,is_active INTEGER DEFAULT 1,must_change_password INTEGER DEFAULT 1,allowed_menus TEXT DEFAULT '["my_dashboard","inout","account"]',device_token TEXT,last_login TEXT,twofa_secret TEXT,twofa_enabled INTEGER DEFAULT 0,FOREIGN KEY(staff_id) REFERENCES staff(id),FOREIGN KEY(office_id) REFERENCES offices(id));
    ...
    migrations = {
      ...
      'users': [('role',"TEXT DEFAULT 'staff'"),('staff_id','INTEGER'),('office_id','INTEGER'),('is_active','INTEGER DEFAULT 1'),('must_change_password','INTEGER DEFAULT 1'),('allowed_menus',"TEXT DEFAULT '[\"my_dashboard\",\"inout\",\"account\"]'"),('device_token','TEXT'),('last_login','TEXT'),('twofa_secret','TEXT'),('twofa_enabled','INTEGER DEFAULT 0')],
...
def office_detail(oid):
 with db_conn() as c:
  ...
  today=now().date().isoformat(); jobs=c.execute('''SELECT a.staff_id,ch.name,ch.category,sh.name shift_name,a.start_time,a.end_time FROM assignments a LEFT JOIN channels ch ON ch.id=a.channel_id LEFT JOIN shifts sh ON sh.id=a.shift_id WHERE a.office_id=? AND a.work_date=? AND a.is_active=1 ORDER BY a.staff_id,ch.category,ch.name''',(oid,today)).fetchall()
  jobmap={}
...
def staff_page():
 with db_conn() as c:
  if request.method=='POST':
   f=request.form; sid=f.get('id',type=int); vals=(f['name'].strip(),f.get('telegram_id') or None,f.get('telegram_username','').strip(),f.get('email','').strip(),f.get('agent_code','').strip(),f.get('cs_name','').strip(),f.get('office_id',type=int),f.get('position','CS'),f.get('status','Aktif'),f.get('join_date') or None,f.get('notes','').strip())
   if sid:
    ...
    c.execute('UPDATE staff SET name=?,telegram_id=?,telegram_username=?,email=?,agent_code=?,cs_name=?,office_id=?,position=?,status=?,join_date=?,notes=? WHERE id=?',vals+(sid,))
   else:
    sid=c.execute('INSERT INTO staff(name,telegram_id,telegram_username,email,agent_code,cs_name,office_id,position,status,join_date,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?)',vals).lastrowid
   if f.get('login_id'):
    ...
    if existing_user:
     ...
     pw=f.get('password') or secrets.token_urlsafe(6); c.execute('INSERT INTO users(username,password_hash,role,staff_id,office_id,is_active,must_change_password,allowed_menus) VALUES(?,?,?,?,?,1,1,?)',(login_id,generate_password_hash(pw),'staff',sid,f.get('office_id',type=int),json.dumps(['my_dashboard','inout','account'])))
   audit(c,'staff.save',f'id={sid}'); c.commit(); flash('Data staf tersimpan.','success'); return redirect(url_for('staff_page'))
  rows=c.execute('SELECT s.*,o.name office_name,u.username login_id,u.is_active account_active FROM staff s LEFT JOIN offices o ON o.id=s.office_id LEFT JOIN users u ON u.staff_id=s.id ORDER BY CASE WHEN s.status="Aktif" THEN 0 ELSE 1 END,s.name').fetchall(); offices=c.execute('SELECT * FROM offices ORDER BY name').fetchall()
...
def staff_detail(sid):
 with db_conn() as c:
  st=c.execute('SELECT s.*,o.name office_name,o.location FROM staff s LEFT JOIN offices o ON o.id=s.office_id WHERE s.id=?',(sid,)).fetchone()
  if not st:return ('Staf tidak ditemukan',404)
  latest_shift=c.execute('''SELECT sh.name,sh.start_time,sh.end_time,ss.work_date FROM shift_schedules ss JOIN shifts sh ON sh.id=ss.shift_id WHERE ss.staff_id=? ORDER BY ss.work_date DESC LIMIT 1''',(sid,)).fetchone()
  today=now().date().isoformat(); jobs=c.execute('''SELECT ch.name,ch.category,sh.name shift_name,a.start_time,a.end_time FROM assignments a LEFT JOIN channels ch ON ch.id=a.channel_id LEFT JOIN shifts sh ON sh.id=a.shift_id WHERE a.staff_id=? AND a.work_date=? AND a.is_active=1 ORDER BY ch.category,ch.name''',(sid,today)).fetchall()
  sp=c.execute('SELECT * FROM warnings WHERE staff_id=? ORDER BY warning_date DESC,id DESC',(sid,)).fetchall()
 return render_template('staff_detail.html',st=st,latest_shift=latest_shift,jobs=jobs,sp=sp,today=today)
...
def offdays_page():
 with db_conn() as c:
  if request.method=='POST':
   f=request.form; oid=f.get('id',type=int); vals=(f.get('staff_id',type=int),f['off_date'],f.get('notes','').strip())
   if oid:c.execute('UPDATE offdays SET staff_id=?,off_date=?,notes=? WHERE id=?',vals+(oid,));audit(c,'offday.update',f'id={oid}')
   else:c.execute('INSERT INTO offdays(staff_id,off_date,notes,created_at) VALUES(?,?,?,?)',vals+(now().isoformat(),));audit(c,'offday.create',f'staff={vals[0]} date={vals[1]}')
   c.commit();flash('Off Day berhasil disimpan.','success');return redirect(url_for('offdays_page'))
  rows=c.execute('SELECT od.*,s.name,s.cs_name,o.name office_name FROM offdays od JOIN staff s ON s.id=od.staff_id LEFT JOIN offices o ON o.id=s.office_id ORDER BY od.off_date DESC,s.name').fetchall(); staff=c.execute("SELECT * FROM staff WHERE status='Aktif' ORDER BY name").fetchall()
 return render_template('offdays.html',rows=rows,staff=staff)

@app.post('/offdays/<int:oid>/delete')
@roles('superadmin','supervisor','leader')
def offday_delete(oid):
 with db_conn() as c:audit(c,'offday.delete',f'id={oid}');c.execute('DELETE FROM offdays WHERE id=?',(oid,));c.commit()
 flash('Off Day dihapus.','success');return redirect(url_for('offdays_page'))
...
def operations_page():
 with db_conn() as c:
  if request.method=='POST':
   f=request.form; action=(f.get('action') or '').strip(); work_date=f.get('work_date') or now().date().isoformat(); office_id=f.get('office_id',type=int)
   try:
    if action=='shift_save':
     ...
     for cid in channel_ids:
      ch=c.execute("SELECT * FROM channels WHERE id=? AND status='Aktif' AND office_id=?",(cid,office_id)).fetchone()
      if ch: valid.append(ch)
     if len(valid)!=len(channel_ids): raise ValueError('Ada jobdesk yang tidak valid untuk kantor ini.')
  ...
  date=request.args.get('date') or now().date().isoformat(); office_id=request.args.get('office_id',type=int)
  ...
  channels=c.execute("SELECT * FROM channels WHERE office_id=? AND status='Aktif' ORDER BY category,name",(office_id,)).fetchall() if office_id else []
 ...
 return render_template('operations.html',date=date,office_id=office_id,offices=offices,shifts=shifts,staff=staff,channels=channels,by_shift=list(by_shift.values()),unassigned=unassigned,edit_row=edit_row)
...
@roles('superadmin','supervisor','leader')
def reports_page():
 report_date=request.args.get('date') or now().date().isoformat(); office_id=request.args.get('office_id',type=int)
 try: day_start=datetime.fromisoformat(report_date+'T00:00:00').replace(tzinfo=WIB); day_end=day_start+timedelta(days=1)
 except ValueError: report_date=now().date().isoformat(); day_start=now().replace(hour=0,minute=0,second=0,microsecond=0); day_end=day_start+timedelta(days=1)
 start_ts=int(day_start.timestamp()); end_ts=int(day_end.timestamp())
 with db_conn() as c:
  offices=c.execute("SELECT * FROM offices WHERE status='Aktif' ORDER BY name").fetchall(); params=[start_ts,end_ts]; office_clause=''
  if office_id: office_clause=' AND s.office_id=?'; params.append(office_id)
  rows=c.execute('''SELECT s.id,s.name,s.cs_name,o.name office_name,COUNT(f.id) total_forms,
      SUM(CASE WHEN f.alert_sent=1 THEN 1 ELSE 0 END) late_forms,
      MAX(CASE WHEN f.first_seen>0 THEN CAST((f.last_seen-f.first_seen)/60 AS INTEGER) ELSE 0 END) max_age
      FROM staff s LEFT JOIN offices o ON o.id=s.office_id
      LEFT JOIN deposit_forms f ON f.staff_id=s.id AND f.first_seen>=? AND f.first_seen<?
      WHERE s.status!='Ex Karyawan' '''+office_clause+''' GROUP BY s.id ORDER BY late_forms DESC,total_forms DESC,s.name''',params).fetchall()
  lparams=[day_start.isoformat(),day_end.isoformat()]; lclause=''
  if office_id:lclause=' AND s.office_id=?';lparams.append(office_id)
  leaves=c.execute('''SELECT s.name,s.cs_name,o.name office_name,COUNT(l.id) total_out,SUM(l.late_minutes) late_minutes,SUM(l.fine) fine FROM staff s LEFT JOIN offices o ON o.id=s.office_id LEFT JOIN leaves l ON l.staff_id=s.id AND l.out_at>=? AND l.out_at<? WHERE s.status!='Ex Karyawan' '''+lclause+''' GROUP BY s.id ORDER BY fine DESC,total_out DESC''',lparams).fetchall()
  totals=c.execute('SELECT COUNT(*) total,SUM(CASE WHEN alert_sent=1 THEN 1 ELSE 0 END) late FROM deposit_forms WHERE first_seen>=? AND first_seen<?'+(' AND office_id=?' if office_id else ''),(start_ts,end_ts,office_id) if office_id else (start_ts,end_ts)).fetchone()
 return render_template('reports.html',rows=rows,leaves=leaves,offices=offices,office_id=office_id,report_date=report_date,totals=totals,retention_days=RETENTION_DAYS)

@app.get('/staff-assignments')
@roles('superadmin','supervisor','leader')
def staff_assignments_page():
 return redirect(url_for('operations_page'))
 date=request.args.get('date') or now().date().isoformat(); office_id=request.args.get('office_id',type=int); shift_id=request.args.get('shift_id',type=int); q=(request.args.get('q') or '').strip()
 with db_conn() as c:
  params=[date]; where='a.work_date=? AND a.is_active=1'
  if office_id:where+=' AND a.office_id=?';params.append(office_id)
  if shift_id:where+=' AND a.shift_id=?';params.append(shift_id)
  if q:where+=' AND (LOWER(s.name) LIKE ? OR LOWER(COALESCE(s.cs_name,\'\')) LIKE ?)';params.extend([f'%{q.lower()}%',f'%{q.lower()}%'])
  raw=c.execute('''SELECT a.*,s.name staff_name,s.cs_name,o.name office_name,sh.name shift_name,ch.name channel_name,ch.category FROM assignments a JOIN staff s ON s.id=a.staff_id LEFT JOIN offices o ON o.id=a.office_id LEFT JOIN shifts sh ON sh.id=a.shift_id LEFT JOIN channels ch ON ch.id=a.channel_id WHERE '''+where+''' ORDER BY ch.category,o.name,sh.start_time,s.name,ch.name''',params).fetchall()
  grouped={}
  for r in raw:
   cat=(r['category'] or 'LAINNYA').upper(); key=(cat,r['staff_id'],r['office_id'],r['shift_id']); item=grouped.setdefault(key,{'category':cat,'staff_name':r['staff_name'],'cs_name':r['cs_name'],'office_name':r['office_name'],'shift_name':r['shift_name'],'jobs':[]}); item['jobs'].append(r['channel_name'] or r['target'] or '-')
  offices=c.execute("SELECT * FROM offices WHERE status='Aktif' ORDER BY name").fetchall(); shifts=c.execute("SELECT * FROM shifts WHERE status='Aktif' ORDER BY office_id,start_time").fetchall()
 return render_template('staff_assignments.html',items=list(grouped.values()),offices=offices,shifts=shifts,date=date,office_id=office_id,shift_id=shift_id,q=q)
  ...
  try:
   with db_conn() as c:
    ...
    if last_cleanup_day!=today_key:
     ...
     c.execute('DELETE FROM leaves WHERE out_at<?',(cutoff_iso,))
